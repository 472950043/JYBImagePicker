...
class AssetViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad() 
        editButton = UIBarButtonItem(barButtonSystemItem: .edit, target: self, action: #selector(editAsset))
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        view.layoutIfNeeded()
        updateImage()
        NotificationCenter.default.addObserver(self, selector: #selector(appWillEnterForegroundNotification), name: Notification.Name.UIApplicationWillEnterForeground, object: nil)
    ...
    @objc func editAsset(_ sender: UIBarButtonItem) {
    ...
    @objc func play(_ sender: UIBarButtonItem) {
        if playerLayer == nil {
            ...
            options.progressHandler = { (progress, error, flag, info) in
                print(progress, error ?? "", flag, info ?? "")
            ...
            PHImageManager.default().requestAVAsset(forVideo: asset, options: options) { (asset, audioMix, info) in
                DispatchQueue.main.sync {
                    ...
                    print(asset ?? "", audioMix ?? "", info ?? "")
                    if let key = asset {
                        // Create an AVPlayer and AVPlayerLayer with the AVPlayerItem.
                        let player = AVPlayer(playerItem: AVPlayerItem(asset: key))
                        let playerLayer = AVPlayerLayer(player: player)
                        
                        // Configure the AVPlayerLayer and add it to the view.
                        playerLayer.videoGravity = AVLayerVideoGravity.resizeAspect
                        playerLayer.frame = self.imageView.layer.frame
                        ...
                        //监控时间进度
                        self.link = CADisplayLink(target: self, selector: #selector(AssetViewController.updateCurrentTime))
    ...
    func playerPlay() {
        ...
        toolbarItems = [favoriteButton, space, pauseButton, space, trashButton]
    ...
    @objc func updateCurrentTime() {
        if playerLayer != nil {
            if let avPlayer = self.playerLayer.player,
                ...
                let top = CMTimeGetSeconds(currentTime)
                let bottom = CMTimeGetSeconds(duration)
                if bottom > 0 {
                    self.progressView.progress = Float(top / bottom)
    ...
    func applyVideoFilter(_ filterName: String, input: PHContentEditingInput, output: PHContentEditingOutput, completion: @escaping () -> ()) {
        ...
        let composition = AVVideoComposition(
            ...
            applyingCIFiltersWithHandler: { request in
                let filtered = request.sourceImage
                    .applyingFilter(filterName)
//                    .applyingFilter(filterName, withInputParameters: nil)
...
extension AssetViewController: PHPhotoLibraryChangeObserver {
    func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        DispatchQueue.main.sync {
            ...
            if let key = details.objectAfterChanges {
                asset = key
            } 
