...
class AssetGridViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        ...
        if self.isScrollBottom { //只在初始化的时候执行
            ...
            DispatchQueue.main.asyncAfter(deadline: popTime) {
                collectionView.scrollToItem(at: IndexPath(row: self.fetchResult.count - 1, section: 0), at: .bottom, animated: false)
